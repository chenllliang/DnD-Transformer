...
def compute_statistics_dataset(dataset,
    ...
    for data_batch in tqdm(loader, desc="compute acts"):
        xs = data_batch[0]

        if len(data_batch) == 3:
            num_codes = data_batch[2]
        ...
        if stage1_model:
            ...
            if len(data_batch) == 3: #vlrq
                xs_recon = torch.cat([
                    stage1_model(imgs[i:i+1],num_codes=num_codes[i:i+1])[0] for i in range(imgs.shape[0])
                ], dim=0)
            else:
                xs_recon = torch.cat([
