...
class VLRQBottleneck(nn.Module):
    """
    Quantization bottleneck via Variable-Length Residual Quantization.
    ...
    def quantize(self, x, num_codes):
        ...
        B_codes, h_codes, w_codes = num_codes.shape
        assert B == B_codes and h == h_codes and w == w_codes

        residual_feature = x.detach().clone()

        max_code_depth = 64 # int(num_codes.max().item())

        quant_list = []
        code_list = []
        aggregated_quants = torch.zeros_like(x)

        for i in range(max_code_depth):
            quant, code = self.codebooks[0](residual_feature)
            # create a mask for the code depth, the mask is 1 if the code depth larger less than i

            code_mask = (num_codes <= i)
            quant_mask = code_mask.unsqueeze(-1).expand_as(quant)
            # fill the quantized feature with zero if the mask is true
            
            code = code.masked_fill(code_mask, -1)
            quant = quant.masked_fill(quant_mask, 0)
    ...
    def forward(self, x, num_codes):
        x_reshaped = self.to_code_shape(x)
        quant_list, codes = self.quantize(x_reshaped, num_codes)
