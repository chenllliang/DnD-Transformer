...
from .quantizations import RQBottleneck, VLRQBottleneck
from .modules import Encoder, Decoder
from .layers import ResnetBlock


class VLRQVAE(Stage1Model):
    def __init__(self,
                 *,
                 embed_dim=64,
                 n_embed=512,
        ...
        if bottleneck_type == 'vlrq':
            ...
            self.quantizer = VLRQBottleneck(latent_shape=latent_shape,
            ...
            raise ValueError("invalid 'bottleneck_type' (must be 'vlrq')")
    ...
    def forward(self, xs, num_codes):
        z_e = self.encode(xs)
        z_q, quant_loss, code = self.quantizer(z_e,num_codes)
        out = self.decode(z_q)
        return out, quant_loss, code

    def forward_code_feature(self, xs, num_codes):
        z_e = self.encode(xs)
        z_q, quant_loss, code = self.quantizer(z_e,num_codes)
        return z_q
