...
class Trainer(TrainerTemplate):
    ...
    def eval(self, valid=True, ema=False, verbose=False, epoch=0):
        ...
        for it, inputs in pbar:
            ...
            # if inputs has the third dim, extract the third dim as the number of codes to use for each patch
            if len(inputs) == 3:
                num_codes = inputs[2].to(self.device, non_blocking=True)
                outputs = model(xs, num_codes=num_codes)
            else:
                outputs = model(xs)
