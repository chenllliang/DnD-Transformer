...
def main(args):
    ...
    for x, y in loader:
        ...
        # first detect whether the code exists
        train_steps = rank + total
        if os.path.exists(f'{args.code_path}/{args.dataset}{args.image_size}_codes/{train_steps}.npy') and os.path.exists(f'{args.code_path}/{args.dataset}{args.image_size}_labels/{train_steps}.npy'):
            print(f"skip {total}")
            if not args.debug:
        ...
        y = y.to(device)
        with torch.no_grad():
            out, quant_loss, code = vq_model(x_all)

        # import pdb; pdb.set_trace()
        

        x = code.detach().cpu().numpy()    # (1, num_aug, args.image_size//16 * args.image_size//16)
        train_steps = rank + total
