...
from llamagen_utils.logger import create_logger
from llamagen_utils.distributed import init_distributed_mode
from llamagen_utils.ema import update_ema, requires_grad
...
def main(args):
    ...
    if rank == 0:
        ...
        logger = create_logger(cloud_results_dir)
        logger.info(f"Experiment directory created at {cloud_results_dir}")
    ...
    logger.info(f"Dataset contains {len(dataset):,} images ({args.feature_dir}) "
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for x, y in loader:
            x = x.to(device, non_blocking=True)[:,:,:,:,:args.train_code_depth]
            y = y.to(device, non_blocking=True)

            z_indices = x.reshape(x.shape[0], -1, x.shape[-1])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--is-flip", action='store_true', help='whether to use flip augmentation')
    ...
    parser.add_argument("--dataset", type=str, default='multilayer_code')
    ...
    parser.add_argument("--train-code-depth", type=int, default=8)
